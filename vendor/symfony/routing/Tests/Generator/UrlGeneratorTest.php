...
class UrlGeneratorTest extends TestCase
{
    ...
    {
        $routes = $this->getRoutes('test', new Route('/testing'));
        $url = $this->getGenerator($routes, array('httpPort' => 8082))->generate('test', array(), UrlGeneratorInterface::ABSOLUTE_URL);

        $this->assertEquals('http://localhost:8082/app.php/testing', $url);
    }

    public function testAbsoluteSecureUrlWithNonStandardPort()
    {
        $routes = $this->getRoutes('test', new Route('/testing'));
        $url = $this->getGenerator($routes, array('httpsPort' => 8082, 'scheme' => 'https'))->generate('test', array(), UrlGeneratorInterface::ABSOLUTE_URL);

        $this->assertEquals('https://localhost:8082/app.php/testing', $url);
    }
