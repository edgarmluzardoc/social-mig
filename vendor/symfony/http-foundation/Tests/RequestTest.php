...
class RequestTest extends TestCase
{
    ...
    public function testCreateCheckPrecedence()
    {
        ...
        $request = Request::create('http://thomas:pokemon@example.net:8082/?foo=bar', 'GET', array(), array(), array(), array(
            'HTTP_HOST' => 'example.com',
            'HTTPS' => 'on',
            'SERVER_PORT' => 443,
        ));
        $this->assertEquals('example.net', $request->getHost());
        $this->assertEquals(8082, $request->getPort());
        $this->assertFalse($request->isSecure());
    ...
    public function testGetUri()
    {
        ...
        // http://host:8082/index.php/path/info?query=string

        $server['HTTP_HOST'] = 'host:8082';
        ...
        $this->assertEquals('http://host:8082/index.php/path/info?query=string', $request->getUri(), '->getUri() with non default port');
        ...
        // http://host:8082/path/info?query=string
        $server = array();
        $server['HTTP_HOST'] = 'host:8082';
        $server['SERVER_NAME'] = 'servername';
        $server['SERVER_PORT'] = '8082';
        ...
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $this->assertEquals('http://host:8082/path/info?query=string', $request->getUri(), '->getUri() with rewrite');
        ...
        $this->assertEquals(
            'http://host:8082/ba%20se/index_dev.php/foo%20bar/in+fo?query=string',
            $request->getUri()
        );
        ...
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $this->assertEquals('http://host:8082/ba%20se/index_dev.php/foo%20bar/in+fo?query=string', $request->getUri());
    ...
    public function testGetUriForPath()
    {
        ...
        $this->assertEquals('http://host:8082/index.php/some/path', $request->getUriForPath('/some/path'), '->getUriForPath() with non default port');
        ...
        $request->initialize(array(), array(), array(), array(), array(), $server);
        $this->assertEquals('http://host:8082/some/path', $request->getUriForPath('/some/path'), '->getUri() with rewrite');
    ...
    public function testGetHost()
    {
        ...
        $request->initialize(array(), array(), array(), array(), array(), array('HTTP_HOST' => 'www.example.com:8082'));
    ...
    public function testTrustedProxiesXForwardedFor()
    {
        ...
        $request->headers->set('X_FORWARDED_HOST', 'foo.example.com:1234, real.example.com:8082');
        $request->headers->set('X_FORWARDED_PROTO', 'https');
    ...
    public function testTrustedProxiesForwarded()
    {
        $request = Request::create('http://example.com/');
        $request->server->set('REMOTE_ADDR', '3.3.3.3');
        $request->headers->set('FORWARDED', 'for=1.1.1.1, host=foo.example.com:8082, proto=https, for=2.2.2.2, host=real.example.com:8082');
    ...
    {
        return array(
            array('',  '', '/', 'http://host:8082/', ''),
            array('/', '', '/', 'http://host:8082/', ''),

            array('hello/app.php/x',  '', '/x', 'http://host:8082/hello/app.php/x', '/hello', '/hello/app.php'),
            array('/hello/app.php/x', '', '/x', 'http://host:8082/hello/app.php/x', '/hello', '/hello/app.php'),

            array('',      'a=b', '/', 'http://host:8082/?a=b'),
            array('?a=b',  'a=b', '/', 'http://host:8082/?a=b'),
            array('/?a=b', 'a=b', '/', 'http://host:8082/?a=b'),

            array('x',      'a=b', '/x', 'http://host:8082/x?a=b'),
            array('x?a=b',  'a=b', '/x', 'http://host:8082/x?a=b'),
            array('/x?a=b', 'a=b', '/x', 'http://host:8082/x?a=b'),

            array('hello/x',  '', '/x', 'http://host:8082/hello/x', '/hello'),
            array('/hello/x', '', '/x', 'http://host:8082/hello/x', '/hello'),

            array('hello/app.php/x',      'a=b', '/x', 'http://host:8082/hello/app.php/x?a=b', '/hello', '/hello/app.php'),
            array('hello/app.php/x?a=b',  'a=b', '/x', 'http://host:8082/hello/app.php/x?a=b', '/hello', '/hello/app.php'),
            array('/hello/app.php/x?a=b', 'a=b', '/x', 'http://host:8082/hello/app.php/x?a=b', '/hello', '/hello/app.php'),
        );
    }
    ...
    public function testNonstandardRequests($requestUri, $queryString, $expectedPathInfo, $expectedUri, $expectedBasePath = '', $expectedBaseUrl = null)
    {
        ...
        $this->assertEquals(8082, $request->getPort());
        $this->assertEquals('host:8082', $request->getHttpHost());
